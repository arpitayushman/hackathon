...
def clean_data(df):
    ...
    # Robust handling of transaction amount
    if 'txn_amount' in cleaned_df.columns:
        ...
        # If conversion results in all NaN, print a warning
        if cleaned_df['txn_amount'].isna().all():
            print("Warning: Unable to convert 'txn_amount' to numeric. Please check the data.")
        else:
...
def create_analysis_data(df):
    ...
    # Ensure 'txn_amount' is included in numeric columns
    if 'txn_amount' not in numeric_columns:
        print("Warning: 'txn_amount' column not found or not numeric. Attempting conversion.")
        try:
            df['txn_amount'] = pd.to_numeric(df['txn_amount'], errors='coerce')
        except Exception as e:
            print(f"Error converting txn_amount: {e}")
            return None, None
